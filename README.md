...
For production Daraja apps with `C2B v2` enabled, the backend registers URLs through Safaricom's production C2B v2 endpoint using `MPESA_C2B_SHORTCODE`. This must be the shortcode shown on the Daraja app at developer.safaricom.co.ke/MyApps — the same shortcode that owns the Daraja credentials. For the Prod-MOBOSOFT ENTERPRISE HQ app, this is `6270335` (the paybill shortcode). STK Push still uses `MPESA_TILL_NUMBER`.
